...
<a href="/top-product" class="view-all ml-2"></a>
</h3>
...
<label for="rasanmart">Sort the product as per your need:</label>
  <select name="rasanmart" id="rasanmart" onchange="location = this.value;" >
       ...
       <option value="/top-product?sort=updated_at&direction=asc">Sort by Latest</option>
       <option value="/top-product?sort=updated_at&direction=asc">Sort by Oldest</option>
       <option value="/top-product?sort=sale_price&direction=asc">Sort by price: Low to high</option>
    <option value="/top-product?sort=sale_price&direction=desc">Sort by price: High to low</option>
    <option value="/top-product?sort=product_name&direction=asc">Sort by Product Name: a-z</option>
    <option value="/top-product?sort=product_name&direction=desc">Sort by Product Name: z-a</option>
...
document.getElementById('product-priceFeature{{$prod->id}}').innerHTML=price;
          ...
          @else   
          <button id="add-cart{{$prod->id}}" type="submit btn-block" class="add-to-cart ml-3"><i class="fa fa-shopping-cart"></i> Add To Cart</button>
         @endif
...
$('#sidebarNavToggler').click();
  ...
  {!! $top_product->appends(\Request::except('page'))->render() !!}
  <!--  Product Modal -->
  @foreach($top_product as $prod)
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body product-grid4">
                     ...
                     <input type="hidden"  name="product_id" value="{{$prod->id}}" />
                            ...
                            <input type="number" id="quantityLatestModal{{$prod->id}}" name="quantity" value="1" style="text-align: right;border: 1px solid #ccc;" readonly/>
                          ...
                          <div id="" class="total-amount">
                              <span style="float: left;margin-left: 125px;margin-top:10px">Price (NRs.)</span>
        <input type="text" id="product-priceLatestModal{{$prod->id}}" name="totalprice" value="{{$prod->sale_price}}" style="border:none;width:21%;margin-right:51px;margin-top:12px">
                          </div>
                                    </div>
                                    <script type="text/javascript">
